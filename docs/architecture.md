...
Note: **no `wp_pattern_keywords` meta is registered** for `tbell_pattern_block`, though the controller reads and writes it. This means keywords are not exposed via REST.
...
### 4.2 Admin Build (`PatternBuilder_Admin.js`)

Mounts `<AdminLandingPage />` on `#pattern-builder-app`.

`AdminLandingPage` is a **documentation splash screen** — a header, a paragraph about the plugin, and two lists of links pointing to `twentybellows.com/pattern-builder-help`. That's it. No functional UI.

The Redux store (`store.js`) exists and imports are wired, but **no component in the admin build uses it**. The store appears to be scaffolding for an admin app that was never built.
...
The Redux store (`src/utils/store.js`) is registered as `'pattern-builder'` but is effectively dead code in the current build:

```js
import {
    deletePattern,
    fetchEditorConfiguration,
    savePattern,  // ← these three do not exist in resolvers.js
    fetchAllPatterns,
} from './resolvers';
```

**`resolvers.js` only exports `fetchAllPatterns`**. The other three imports (`deletePattern`, `fetchEditorConfiguration`, `savePattern`) are undefined, which would produce runtime `TypeError` if any of the store's thunk actions that call them were ever dispatched. Since no component dispatches those actions today, this is silent dead code — but it's a landmine.
...
### 13.1 🔴 Critical: `array_find()` requires PHP 8.4
...
**Fix:** Replace with `array_filter()` + `reset()` or a manual loop.

```php
// Replace:
$pattern = array_find($patterns, fn($p) => $p->name === $pattern->name);

// With:
$filtered = array_filter($patterns, fn($p) => $p->name === $pattern->name);
$pattern = reset($filtered) ?: null;
```

### 13.2 🔴 Critical: `store.js` imports non-existent functions
...
Currently, no component dispatches these actions, so this is silent dead code. But it will explode if anyone wires up the store in the future. Either implement the missing resolvers or remove the dead store actions.

### 13.3 🟡 High: Capabilities assigned on every `init`
...
`WP_Role::add_cap()` calls `update_option()` if the capability isn't already stored. On the first run this is a DB write per capability per role. On subsequent runs it's a read+no-op. But checking it on every `init` (every request) is wasteful. This should run on plugin activation via `register_activation_hook`.

### 13.4 🟡 High: DB upsert on every page load
...
### 13.5 🟡 High: `tbell_pattern_block` is `public: true`
...
`public: true` means WordPress will create rewrite rules for this post type. `tbell_pattern_block` posts will have public URLs like `/tbell_pattern_block/my-pattern/`. Visiting one will try to render the pattern content directly. This is almost certainly unintended. Should be `public: false` with explicit `show_in_rest: true`.

### 13.6 🟡 High: Rules of Hooks violation in `syncedPatternFilter`
...
React's Rules of Hooks forbid calling hooks inside conditionals. This pattern will work only as long as every rendered instance goes through the same branch on every render — which is not guaranteed when React reconciles the component tree. This can cause stale state, missed updates, or outright crashes in edge cases. The hook should be called unconditionally, with the conditional logic applied to the returned value.

### 13.7 🟡 Medium: Admin page is an empty shell

`PatternBuilder_Admin.js` renders a welcome page with documentation links. It occupies a top-level admin menu entry under Appearance but provides no functionality. The admin JS build, its asset loading, and its Sass are overhead for something that is purely a link list. This should either be filled with real functionality or replaced with a simpler PHP-rendered page.

### 13.8 🟡 Medium: Slug encoding sentinel `-x-x-` is not escaped
...
### 13.9 🟡 Medium: `get_block_patterns_from_theme_files()` ignores parent theme
...
### 13.10 🟡 Medium: No refresh after initial pattern fetch in EditorSidePanel
...
### 14.3 The Admin App Is Unfinished

Looking at the Redux store, it has `activePattern`, `editorConfiguration`, `allPatterns`, `filterOptions` — the skeleton of a standalone pattern management app. The admin page under Appearance was likely planned to be a full pattern manager (list all patterns, preview, edit, create, delete) similar to what exists in the editor sidebar. It was either not built yet or was removed and replaced by moving everything into the editor sidebar.

This explains the dead `store.js` code and the `resolvers.js` stubs. It's worth deciding: either build the admin app, or delete `store.js` and the admin build assets to reduce the binary footprint.
...
| PHP code quality | ⚠️ Issues | `array_find()` PHP 8.4 bug is critical; `public: true` CPT needs fixing |
| JS architecture | ⚠️ Issues | Dead store, broken resolver imports, hooks violation |
| JS test coverage | ❌ None | Unit tests are a placeholder |
| Performance | ⚠️ Issues | DB upsert on every page load; capability assignment on every init |
| Admin UI | ⚠️ Incomplete | Admin page is empty; store is built for something not yet implemented |
| Companion plugin | ✅ Good | Clean relationship; appropriate read-only subset |

### Top 5 Things to Fix Now

1. **Replace `array_find()` with a PHP 7.2-compatible alternative** — it's a production blocker on most hosting environments.
2. **Either implement `deletePattern`/`savePattern`/`fetchEditorConfiguration` in `resolvers.js` or remove the dead store code** — the codebase has misleading dead code.
3. **Fix the Rules of Hooks violation in `syncedPatternFilter`** — call `useSelect` unconditionally.
4. **Move capability assignment to `register_activation_hook`** — not `init`.
5. **Change `tbell_pattern_block` to `public: false`** — patterns shouldn't have public URLs.

### Top 3 Longer-Term Improvements

1. **Cache the pattern registration** — use transients keyed by `filemtime()` hashes of the pattern directory to skip the DB upsert loop on warm requests.
2. **Fix `PatternCreatePanel` to handle `source: 'theme'` on creation** — either intercept the POST in PHP or handle it as a POST+PUT sequence in JS.
3. **Decide the admin app's fate** — build it or remove it. The current state (welcome screen + dead store infrastructure) is worse than either extreme.
