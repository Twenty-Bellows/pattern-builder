...
class Pattern_Builder_Controller {
	...
	public function update_theme_pattern( Abstract_Pattern $pattern ) {
		if ( pb_fs()->can_use_premium_code__premium_only() || pb_fs_testing() ) {
			...
			$pattern = $this->localize_pattern_content( $pattern );
	 ...
	 */
	private function localize_blocks( $blocks ) {
		foreach ( $blocks as &$block ) {
			...
			switch ( $block['blockName'] ) {
				...
				case 'core/button':
					$block = $this->localize_button_block( $block );
					break;

				case 'core/image':
					$block = $this->localize_image_block( $block );
					break;

				case 'core/cover':
				case 'core/media-text':
					$block = $this->localize_media_block( $block );
					break;
			...
			if ( ! empty( $block['innerBlocks'] ) ) {
				$block['innerBlocks'] = $this->localize_blocks( $block['innerBlocks'] );
			}
		}
	 ...
	 * @return array Localized block.
	 */
	private function localize_button_block( $block ) {
		if ( ! empty( $block['innerHTML'] ) ) {
			...
			if ( preg_match( '/<a[^>]*>(.*?)<\/a>/s', $block['innerHTML'], $matches ) ) {
				...
				if ( ! empty( trim( $button_text ) ) ) {
					$localized = $this->create_localized_string( $button_text );
	 ...
	 * @return array Localized block.
	 */
	private function localize_image_block( $block ) {
		...
		if ( ! empty( $block['innerHTML'] ) && strpos( $block['innerHTML'], '<figcaption' ) !== false ) {
			if ( preg_match( '/<figcaption[^>]*>(.*?)<\/figcaption>/s', $block['innerHTML'], $matches ) ) {
				...
				if ( ! empty( trim( $caption ) ) ) {
					$localized = $this->create_localized_string( $caption );
					$block['innerHTML'] = str_replace( '>' . $matches[1] . '<', '>' . $localized . '<', $block['innerHTML'] );
					$block['innerContent'] = array( $block['innerHTML'] );
				}
			}
		...
		if ( ! empty( $block['innerHTML'] ) && preg_match( '/alt="([^"]*)"/', $block['innerHTML'], $matches ) ) {
			...
			if ( ! empty( trim( $alt_text ) ) ) {
				$localized_alt = $this->create_localized_string( $alt_text, 'esc_attr__' );
	 ...
	 * @return array Localized block.
	 */
	private function localize_media_block( $block ) {
		// Localize alt text for media blocks.
		if ( ! empty( $block['attrs']['alt'] ) ) {
			$block['attrs']['alt'] = $this->create_localized_string( $block['attrs']['alt'], 'esc_attr__' );
		}
	 ...
	 * @return array Localized block.
	 */
	private function localize_table_block( $block ) {
		if ( ! empty( $block['innerHTML'] ) ) {
			...
			$block['innerHTML'] = preg_replace_callback(
				...
				function ( $matches ) {
					if ( ! empty( trim( $matches[1] ) ) ) {
						return str_replace( $matches[1], $this->create_localized_string( $matches[1] ), $matches[0] );
					}
					return $matches[0];
				},
				$block['innerHTML']
	...
	public function update_user_pattern( Abstract_Pattern $pattern ) {
		...
		if ( $convert_from_theme_pattern && ! pb_fs()->can_use_premium_code() && ! pb_fs_testing() ) {
			return new WP_Error( 'premium_required', 'Converting Theme Patterns to User Patterns requires the premium version of Pattern Builder.', array( 'status' => 403 ) );
		}
