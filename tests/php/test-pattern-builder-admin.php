...
class Test_Pattern_Builder_Admin extends WP_UnitTestCase {
	...
	/**
	 * Runs settings through every filter the plugin registers.
	 *
	 ...
	 */
	private function filter_settings( WP_Block_Editor_Context $context, array $settings = array() ): array {
		return apply_filters( 'block_editor_settings_all', $settings, $context );
	}
	...
	public function test_leaves_the_other_settings_alone() {
		...
		$settings = $this->filter_settings(
			$this->editor_context(),
			array( 'pbTestSetting' => 'kept' )
		);

		$this->assertSame( 'kept', $settings['pbTestSetting'] );
	}
}
