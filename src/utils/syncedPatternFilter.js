 ...
 *
 * Note: useSelect must be called unconditionally (Rules of Hooks). The conditional logic
 * is applied to the hook's returned value, not to whether the hook is called.
 *
 */
export const syncedPatternFilter = ( BlockEdit ) => ( props ) => {
	const { name, attributes } = props;

	// useSelect is always called — never inside a conditional.
	const selectedPattern = useSelect(
		( select ) => {
			if ( name !== 'core/pattern' || ! attributes.slug ) {
				return null;
			}
			return select( blockEditorStore ).__experimentalGetParsedPattern(
				attributes.slug
			);
		},
